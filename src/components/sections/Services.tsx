...
const FEATURED_TITLES = [
  ...
  'Daniel y las hormigas millonarias',
  'La Mujer que prospera',
];

const featuredCourses = FEATURED_TITLES.map(
  (title) => courses.find((c) => c.title === title)!
);

const ITEMS_PER_PAGE = 3;
const totalPages = Math.ceil(featuredCourses.length / ITEMS_PER_PAGE);
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`rounded-full transition-all duration-200 ${
                currentPage === i ? 'w-4 h-4 bg-navy' : 'w-4 h-4 bg-navy/20'
              }`}
            />
          ))}
        </div>
